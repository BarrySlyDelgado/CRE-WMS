...
def main():
    ...
    for i in range(args.producer_ratio):
        ...
        t = vine.PythonTask(write_file, f"filename_{i}", args.bytes)
        t.add_output(outfile, f"filename_{i}")
        t.set_cores(1)
    ...
    for infile in temp_files:
        for _ in range(args.consumer_ratio):
            t = vine.PythonTask(read_file, "filename")
            t.add_input(infile, "filename")
            t.set_cores(1)
            t.set_category("read")
            m.submit(t)
